...
export default function DoctorSubscription() {
 ...
 {plans.map((plan) => (
 <div key={plan.name} className={`relative bg-slate-900 rounded-3xl p-8 border ${plan.popular ? 'border-emerald-500 shadow-2xl shadow-emerald-500 transform md:-translate-y-4' : 'border-white/[0.08]'} flex flex-col`}>
 {plan.popular && (
 <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1">
 ...
 <button onClick={() => { setSelectedPlan(plan.name); setShowModal(true); }}
 className={`w-full py-3.5 rounded-xl font-bold transition-all ${plan.popular ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/25' : 'bg-white/[0.05] hover:bg-white/[0.1] text-white border border-white/[0.1]'}`}
 >
 Select Plan
 </button>
 </div>
 ...
 <input type="email" placeholder="Email Address" required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
 <div className="relative">
 <CreditCard className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
 <input type="text" placeholder="Card Number" required className="w-full pl-12 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
 </div>
 <div className="flex gap-3">
 <input type="text" placeholder="MM/YY" required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
 <input type="text" placeholder="CVC" required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
 </div>
 </div>
 ...
 <Link href="/doctor/dashboard" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-8 py-3.5 rounded-xl transition-colors shadow-lg shadow-emerald-600/20 block w-full">
 Go to Dashboard
 </Link>
 </div>
 )}
