  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="max-w-2xl w-full bg-white rounded-[3rem] p-10 shadow-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300">
        ...
        {/* AI Quick Entry */}
        <div className="mb-10 p-6 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-[2rem] border border-indigo-100 shadow-inner">
          <label className="block text-[11px] font-black text-indigo-400 uppercase tracking-widest mb-3 ml-2">AI Smart Entry</label>
          <div className="flex gap-3">
            <input 
              type="text" 
              placeholder="e.g. Paracetamol 500mg twice a day for 5 days..."
              value={aiInput}
              onChange={(e) => setAiInput(e.target.value)}
              className="flex-1 bg-white border border-indigo-100 rounded-2xl px-5 py-4 text-sm font-bold text-gray-700 placeholder:text-gray-300 focus:ring-4 focus:ring-indigo-100 outline-none transition-all"
            />
            <button 
              onClick={handleAiEntry}
              disabled={loading || !aiInput.trim()}
              className="bg-indigo-600 text-white font-black px-6 py-4 rounded-2xl text-xs hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 disabled:opacity-50 active:scale-95 uppercase tracking-widest"
            >
              {loading ? "..." : "Magic"}
            </button>
          </div>
