    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-7 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4">
          ...
          <input type="datetime-local" value={datetime} min={minDt} onChange={(e) => setDatetime(e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 bg-gray-50" />
        </div>
        <div className="mb-6">
          <label className="block text-sm font-semibold text-gray-700 mb-2">📝 Notes</label>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="e.g. headache follow-up..." className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 bg-gray-50 resize-none" />
        </div>
        <button disabled={!datetime} onClick={() => onConfirm({ scheduledAt: new Date(datetime).toISOString(), notes })} className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed shadow-lg">📞 Schedule Call</button>
      </div>
    </div>
  );
}
        ...
        <div className="flex-1 glass rounded-2xl p-4 flex flex-col items-center justify-center min-h-[400px] relative">
          ...
          {voice.isActive && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-fade-in">
              ...
              <button
                onClick={voice.stop}
                className="text-xs text-red-400 hover:text-red-600 bg-white/80 backdrop-blur px-3 py-1.5 rounded-full border border-red-200 transition-colors"
              >
                ✕ End Conversation
              </button>
            </div>
